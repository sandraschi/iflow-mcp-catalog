...
# PyInstaller spec — produce iflow-mcp-catalog-backend.exe for Tauri NSIS embedding.
# Fleet standard: strip=False, upx=False, noarchive=True (see tauri_nsis_building.md).
# Usage (from repo root):
#   uv run pyinstaller iflow-mcp-catalog-backend.spec --distpath dist --clean --noconfirm

block_cipher = None

a = Analysis(
    ["src/mcp_catalog/__main__.py"],
    pathex=["src"],
    binaries=[],
    datas=[("src/mcp_catalog", "mcp_catalog")],
    hiddenimports=[
        "uvicorn.logging",
        "uvicorn.loops",
...
exe = EXE(
    ...
    a.datas,
    [],
    name="iflow-mcp-catalog-backend",
    debug=False,
