...
mcp = FastMCP(
    "iflow-mcp-catalog",
    instructions=(
        "Indexes any GitHub org or personal account (default: sandraschi) into a star-sorted, "
        "categorized MCP-oriented catalog. Auto-detects org vs user accounts. "
        "Refresh writes data/catalog.json and reports/catalog_standalone.html. "
...
async def iflow_catalog_refresh(
    ...
    """IFLOW_CATALOG_REFRESH - Fetch repos via GitHub API, classify, persist JSON (+ optional HTML).

    Works with both GitHub organisations and personal accounts -- auto-detected.
    ...
    Args:
        org: GitHub organisation or personal account login (default: sandraschi).
        max_pages: Cap pagination for testing (None = all pages).
        write_html: Also emit reports/catalog_standalone.html.

    Returns:
        Success payload with paths, counts, account_type, and rate-limit hints; or structured error.
    """
    ...
    try:
        ...
        return {
            ...
            "result": {
                "org": org,
                "account_type": diag.get("account_type"),
